...
| Audit log | Ghi lại lịch sử đổi trạng thái work order (`WorkOrderStatusHistory`) để tra cứu khi có tranh chấp — không transition nào được bỏ qua bước ghi log này. Mọi thay đổi `CommissionSplitPercent` trên `WorkOrderAssignment` và mọi quyết định (auto-approve/Admin duyệt/từ chối) trên `ChangeRequest` cũng phải ghi `AuditLog` — đây là dữ liệu liên quan trực tiếp tới lương/tranh chấp chi phí. |
| Toàn vẹn nghiệp vụ (v5) | Không cho reassign Technician sau khi WorkOrder đã `Completed`; không cho auto-assign gán Technician mà bỏ qua điều kiện Bay; không cho `FinalEstimatedDate` nhỏ hơn `SystemSuggestedDate`. |
...
- [ ] Nếu liên quan `WorkOrderAssignment`/`ChangeRequest`: có test case chặn đóng WorkOrder khi tổng `CommissionSplitPercent` ≠ 100%, và test case `ChangeRequest` vượt ngưỡng bị chặn auto-approve (phải qua Admin) không?
